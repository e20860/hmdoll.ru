 ...
 */class ErrorHandler {
    ...
    {
        $logger = new Logger('erhan');
        $logger->pushHandler(new StreamHandler(WWW.'/applogs/errors.log', Logger::WARNING));
        $errstr = "ошибка: {$message}, файл: {$file}, строка: {$line}";
        $logger->err($errstr);
        // Старый шеф протокола
